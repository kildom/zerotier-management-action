...
function parseOr(tokenizer) {
  let a = parseAnd(tokenizer);
  if (tokenizer.match("OR")) {
    let b = parseOr(tokenizer);
    return (n) => {
      return a(n) || b(n);
    };
  } else {
...
function parseAnd(tokenizer) {
  let a = parseLeaf(tokenizer);
  if (tokenizer.match("AND")) {
    let b = parseAnd(tokenizer);
    return (n) => {
      return a(n) && b(n);
    };
  } else {
...
function parseLeaf(tokenizer) {
  if (tokenizer.match("NOT")) {
    let x = parseLeaf(tokenizer);
    return (n) => {
      return !x(n);
    };
  }
...
function compileSelectors(text, fields) {
  ...
  while (!tokenizer.match(END_OF_QUERY)) {
    let selector = parseOr(tokenizer);
  ...
  do {
    ...
    for (let node of list) {
      ...
      for (let cap of ((_b = node.config) == null ? void 0 : _b.capabilities) || []) {
        ...
        for (let [name, id] of Object.entries((networkInfo == null ? void 0 : networkInfo.capabilitiesByName) || {})) {
          if (id == cap) {
      ...
      for (let [id, value] of ((_c = node.config) == null ? void 0 : _c.tags) || []) {
        ...
        for (let [name, info] of Object.entries((networkInfo == null ? void 0 : networkInfo.tagsByName) || {})) {
          if ((info == null ? void 0 : info.id) === id) {
            attr[`tag:${name}`] = value.toString();
            for (let [enumName, enumValue] of Object.entries((info == null ? void 0 : info.enums) || {})) {
              if (enumValue === value) {
    ...
    let matched = selectors.map(() => null);
...
async function writeNodeData() {
  ...
  if (inputs.capabilities.length) {
    ...
    for (let cap of inputs.capabilities) {
      let capInt;
      if (networkInfo.capabilitiesByName && networkInfo.capabilitiesByName[cap] !== void 0) {
  ...
  if (inputs.tags.length) {
    data.config.tags = [];
    let tagsPairs = inputs.tags.map((tag) => tag.split("=", 2));
    for (let [key, value] of tagsPairs) {
      let keyInt;
      let valueInt;
      if (networkInfo.tagsByName && networkInfo.tagsByName[key] !== void 0) {
        let tagInfo = networkInfo.tagsByName[key];
        keyInt = tagInfo.id;
        if (tagInfo.enums && tagInfo.enums[value] !== void 0) {
